...
**Data flow:** `game.ts` exports the mutable `g` object — all modules read/write shared state through it. `constants.ts` holds all tuning numbers. `update.ts` is the largest file (~1300 lines) containing the core game loop registered via `scene.registerBeforeRender`.
...
- **Weapon fires two modes:** Left-click = hitscan beam (raycast), right-click = orb projectile (physics-simulated sphere with explosion splash damage). Both share the ammo pool and heat system.
